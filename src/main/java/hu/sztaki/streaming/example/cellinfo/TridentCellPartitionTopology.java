...
public class TridentCellPartitionTopology {
	...
	private static void wrongArgs() {
		System.out.println("------------------");
		System.out
				.println("Usage: \"{storm jar, scripts/stormrun.sh} <cluster> <unitLength>"
	...
	public static void main(String[] args) throws Exception {
		...
		if (args == null || args.length != 12) {
			...
			TridentState cellCounts = topology
					.newStream("infoSpout",
							new TridentInfoBatchSpout(infoSpoutBatch, infoSpoutSleep, numOfCells))
					.parallelismHint(infoSpoutParallelism)
					.groupBy(new Fields("cellid", "time"))
					.persistentAggregate(new MemoryMapState.Factory(), new Count(),
							new Fields("count")).parallelismHint(infoSpoutParallelism);

			topology.newStream(
					"querySpout",
					new TridentQueryBatchSpout(querySpoutBatch, querySpoutSleep, numOfCells,
							queryLastMillis))
					.parallelismHint(querySpoutParallelism)
					.each(new Fields("timestamp", "lastmillis"), new getTimes(), new Fields("time"))
					.groupBy(new Fields("cellid", "time"))
					.stateQuery(cellCounts, new Fields("cellid", "time"), new MapGet(),
							new Fields("count")).each(new Fields("count"), new FilterNull())
